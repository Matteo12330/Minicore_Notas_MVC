...
using Minicore_Notas_MVC.Models.ViewModel;
...
namespace Minicore_Notas_MVC.Controllers
{
    public class HomeController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult DefineRange([Bind("Start", "End")] GradeVM gradeVM)
        {
            ViewData["StudentId"] = new SelectList(_context.Students, "Id", "Name");
        ...
        public double GetGradesByStudent(int studentId, DateOnly start, DateOnly end, int periodId)
        {
            ...
            var lista = _context.Grades
                .Where(g => g.StudentId == studentId &&
                            g.Date >= start && g.Date <= end &&
                            g.Date >= period.Start && g.Date <= period.End)
                .ToList();
